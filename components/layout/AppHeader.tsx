...
export function AppHeader({ className, onOpenMobileNav }: AppHeaderProps) {
  return (
    <header
      className={cn(
        "sticky top-0 z-[100] flex w-full shrink-0 items-center gap-2 border-b border-white/[0.06] px-3 shadow-[0_6px_24px_-8px_rgba(0,0,0,0.55)] backdrop-blur-[2px] md:gap-3 md:px-4 lg:px-6",
    ...
    >
      {onOpenMobileNav ? (
        <button
          ...
          className="mr-1 shrink-0 rounded-md p-1.5 text-slate-400 transition-colors hover:bg-white/5 hover:text-white lg:hidden"
      ...
      >
        <Image
          ...
          height={120}
          priority
          className="h-[52px] w-auto max-w-[min(260px,42vw)] object-contain object-left sm:h-[64px] md:h-[72px]"
        />
      </Link>

      <div className="mx-auto hidden min-w-0 max-w-xl flex-1 px-2 md:flex">
        <div className="relative w-full">
          ...
          <Input
            ...
            className={cn(
              "h-9 border-white/10 bg-white/[0.06] pl-9 text-sm text-kp-on-surface",
      ...
      <div className="flex min-w-0 flex-1 items-center justify-end gap-0.5 md:min-w-[auto] md:flex-none md:gap-1">
        <button
          type="button"
